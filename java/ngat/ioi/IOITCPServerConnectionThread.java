// IOITCPServerConnectionThread.java
// $Header$
package ngat.ioi;
...
/**
 * This class extends the TCPServerConnectionThread class for the IOI application.
 * @author Chris Mottram
 * @version $Revision: 1.1 $
 */
public class IOITCPServerConnectionThread extends TCPServerConnectionThread
{
	/**
	 ...
	 * This can be retrieved from the IO:I main object, which has a Hashtable with mappings between
	 * ngat.message class names and implementations of these commands.
	 * @see JMSCommandImplementation
	 * @see IOI#getImplementation
	 */
	 ...
	 */
	public IOITCPServerConnectionThread(Socket connectionSocket)
	{
	 ...
	 * stored in the IOI object. It sets up the implementation objects references to the IOI main
	 ...
	 * @see IOI#getImplementation
	 * @see CommandImplementation#setIOI
	...
	// set the threads priority
		if(command instanceof INTERRUPT)
			this.setPriority(ioi.getStatus().getThreadPriorityInterrupt());
		else
	...
	// setup a generic done object until the command specific one is constructed.
		...
		if(command == null)
		{
			ioi.error("processCommand:command was null.");
			done.setErrorNum(IOIConstants.IOI_ERROR_CODE_BASE+100);
	...
	// the status's current command is reset to null.
		catch(Exception e)
		{
			...
			ioi.error(s,e);
			done.setErrorNum(IOIConstants.IOI_ERROR_CODE_BASE+102);
			done.setErrorString(s+e);
			done.setSuccessful(false);
		}
	// change IO:I status once command has been done
		if(!(command instanceof INTERRUPT))
		{
	...
	// log command/done
		ioi.log(Logging.VERBOSITY_VERY_TERSE,"Command:"+command.getClass().getName()+
			" Completed.");
		ioi.log(Logging.VERBOSITY_TERSE,"Done:"+done.getClass().getName()+
