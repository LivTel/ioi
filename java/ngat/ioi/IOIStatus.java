...
public class IOIStatus
{
	...
	/**
	 * Default filename containing network properties for IO:I.
	 */
	private final static String DEFAULT_NET_PROPERTY_FILE_NAME = "./ioi.net.properties";
	/**
	 * Default filename containing properties for IO:I.
	 */
	private final static String DEFAULT_PROPERTY_FILE_NAME = "./ioi.properties";
	/**
	 * Default filename containing FITS properties for IO:I.
	 */
	...
	/**
	 * The name of the ngat.phase2.IOIConfig object instance that was last used	
	 * to configure the instrument (via an ngat.message.ISS_INST.CONFIG message).
	 * Used for the CONFNAME FITS keyword value.
	 * Initialised to 'UNKNOWN', so that if we try to take a frame before configuring the IO:I
	...
	/**
	 * A copy of the last keyword/value hashtable generated from a GetConfig command to the IDL socket server.
	 */
	private Hashtable getConfigCommandCache = null;
	/**
	 * A timestamp of when the GetConfig command hastable was last cached.
	 */
	...
	/**
	 * Set the logging level for IO:I.
	...
	/**
	 * Get the logging level for IO:I.
	...
	/**
	 * Get the the command the IO:I is currently processing.
	...
	/**
	 * Method to change (increment) the unique ID number of the last ngat.phase2.IOIConfig instance to 
	 * successfully configure the IO:I camera.
	 ...
	 * ngat.phase2.IOIConfig instance to successfully configure the IO:I camera.
	 ...
	 * ngat.phase2.IOIConfig instance to successfully configure the IO:I camera.
	 * @return The string identifier, or null if the IO:I camera has not been configured
	 * 	since the IO:I started.
	 * @see #configName
	 */
	...
	/**
	 * Cache a copy of the keyword/value Hastable generated by a recent GetConfig command.
	 * @param h The Hashtable containing the keyword/value pairs from a recent GetConfig command.
	 * @param timestamp The moment in time this data was retrieved.
	 * @see #getConfigCommandCache
	 * @see #getConfigCommandCacheTimestamp
	 */
	public void cacheGetConfigCommand(Hashtable h,Date timestamp)
	{
		getConfigCommandCache = h;
		getConfigCommandCacheTimestamp = timestamp;
	}

	/**
	 * Get a cached copy of the last keyword/value Hastable generated by a GetConfig command.
	 * @return The cached keyword/value hashtable.
	 * @see #getConfigCommandCache
	 */
	public Hashtable getCachedGetConfigCommand()
	{
		return getConfigCommandCache;
	}

	/**
	 * Get a timestamp of when the cached copy of the last keyword/value Hastable generated by a GetConfig command
	 * was generated..
	 * @return A Date containing the timestamp.
	 * @see #getConfigCommandCacheTimestamp
	 */
	public Date getCachedGetConfigCommandTimestamp()
	{
		return getConfigCommandCacheTimestamp;
	}
	 ...
	 * The value is retrieved from the <b>ioi.thread.priority.server</b> property.
	 * If this fails the default IOI_DEFAULT_THREAD_PRIORITY_SERVER is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see IOIConstants#IOI_DEFAULT_THREAD_PRIORITY_SERVER
	 */
	public int getThreadPriorityServer()
	{
		...
		try
		{
			retval = getPropertyInteger("ioi.thread.priority.server");
		...
		{
			retval = IOIConstants.IOI_DEFAULT_THREAD_PRIORITY_SERVER;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>ioi.thread.priority.interrupt</b> property.
	 * If this fails the default IOI_DEFAULT_THREAD_PRIORITY_INTERRUPT is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see IOIConstants#IOI_DEFAULT_THREAD_PRIORITY_INTERRUPT
	 */
	public int getThreadPriorityInterrupt()
	{
		...
		try
		{
			retval = getPropertyInteger("ioi.thread.priority.interrupt");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = IOIConstants.IOI_DEFAULT_THREAD_PRIORITY_INTERRUPT;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>ioi.thread.priority.normal</b> property.
	 * If this fails the default IOI_DEFAULT_THREAD_PRIORITY_NORMAL is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see IOIConstants#IOI_DEFAULT_THREAD_PRIORITY_NORMAL
	 */
	public int getThreadPriorityNormal()
	{
		...
		try
		{
			retval = getPropertyInteger("ioi.thread.priority.normal");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = IOIConstants.IOI_DEFAULT_THREAD_PRIORITY_NORMAL;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>ioi.thread.priority.tit</b> property.
	 * If this fails the default IOI_DEFAULT_THREAD_PRIORITY_TIT is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see IOIConstants#IOI_DEFAULT_THREAD_PRIORITY_TIT
	 */
	public int getThreadPriorityTIT()
	{
		...
		try
		{
			retval = getPropertyInteger("ioi.thread.priority.tit");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = IOIConstants.IOI_DEFAULT_THREAD_PRIORITY_TIT;
		}
		return retval;
	}
	 ...
	 * This is got from the <i>ioi.config.unique_id_filename</i> property.
	...
	private void initialiseConfigId()
	{
		...
		fileName = getProperty("ioi.config.unique_id_filename");
